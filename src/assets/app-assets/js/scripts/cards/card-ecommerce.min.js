! function(e, a, t) {
    "use strict";
    require.config({
        paths: {
            echarts: "../../../app-assets/vendors/js/charts/echarts"
        }
    }), require(["echarts", "echarts/chart/pie", "echarts/chart/funnel"], function(l) {
        var n = l.init(a.getElementById("nightingale-rose-labels"));
        n.setOption({
            title: {
                text: "Monthly Sales",
                subtext: "product sales on monthly basis",
                x: "center",
                textStyle: {
                    color: "#FFF"
                },
                subtextStyle: {
                    color: "#FFF"
                }
            },
            tooltip: {
                trigger: "item",
                formatter: "{a} <br/>{b}: +{c}$ ({d}%)"
            },
            color: ["#ffd775", "#ff847c", "#e84a5f", "#2a363b", "#7fd5c3", "#61a781", "#f0c75e", "#df8c7d", "#e8ed8a", "#55bcbb", "#e974b9", "#2f9395"],
            toolbox: {
                show: !0,
                orient: "vertical",
                feature: {
                    mark: {
                        show: !0,
                        title: {
                            mark: "Markline switch",
                            markUndo: "Undo markline",
                            markClear: "Clear markline"
                        }
                    },
                    dataView: {
                        show: !0,
                        readOnly: !1,
                        title: "View data",
                        lang: ["View chart data", "Close", "Update"]
                    },
                    magicType: {
                        show: !0,
                        title: {
                            pie: "Switch to pies",
                            funnel: "Switch to funnel"
                        },
                        type: ["pie", "funnel"]
                    },
                    restore: {
                        show: !0,
                        title: "Restore"
                    },
                    saveAsImage: {
                        show: !0,
                        title: "Same as image",
                        lang: ["Save"]
                    }
                },
                color: "#FFF"
            },
            calculable: !0,
            series: [{
                name: "Increase (brutto)",
                type: "pie",
                radius: ["15%", "73%"],
                center: ["50%", "57%"],
                roseType: "area",
                itemStyle: {
                    normal: {
                        label: {
                            textStyle: {
                                color: "#FFF"
                            }
                        },
                        labelLine: {
                            lineStyle: {
                                color: "#FFF"
                            }
                        }
                    }
                },
                width: "40%",
                height: "78%",
                x: "30%",
                y: "17.5%",
                max: 450,
                sort: "ascending",
                data: [{
                    value: 440,
                    name: "January"
                }, {
                    value: 260,
                    name: "February"
                }, {
                    value: 350,
                    name: "March"
                }, {
                    value: 250,
                    name: "April"
                }, {
                    value: 210,
                    name: "May"
                }, {
                    value: 350,
                    name: "June"
                }, {
                    value: 300,
                    name: "July"
                }, {
                    value: 430,
                    name: "August"
                }, {
                    value: 400,
                    name: "September"
                }, {
                    value: 450,
                    name: "October"
                }, {
                    value: 330,
                    name: "November"
                }, {
                    value: 200,
                    name: "December"
                }]
            }]
        }), t(function() {
            function a() {
                setTimeout(function() {
                    n.resize()
                }, 200)
            }
            t(e).on("resize", a), t(".menu-toggle").on("click", a)
        });
        var r = l.init(a.getElementById("doughnut"));
        r.setOption({
            title: {
                text: "Top 5 Categories",
                subtext: "Top selling mobiles",
                x: "center",
                textStyle: {
                    color: "#FFF"
                },
                subtextStyle: {
                    color: "#FFF"
                }
            },
            legend: {
                orient: "vertical",
                x: "left",
                data: ["Moto Z", "Galaxy S7 Edge", "One Plus 3", "Mi 5", "iPhone 6s"],
                textStyle: {
                    color: "#FFF"
                }
            },
            color: ["#ffd322", "#ffa422", "#e89805", "#ffc107", "#fff306"],
            toolbox: {
                show: !0,
                orient: "vertical",
                feature: {
                    mark: {
                        show: !0,
                        title: {
                            mark: "Markline switch",
                            markUndo: "Undo markline",
                            markClear: "Clear markline"
                        }
                    },
                    dataView: {
                        show: !0,
                        readOnly: !1,
                        title: "View data",
                        lang: ["View chart data", "Close", "Update"]
                    },
                    magicType: {
                        show: !0,
                        title: {
                            pie: "Switch to pies",
                            funnel: "Switch to funnel"
                        },
                        type: ["pie", "funnel"],
                        option: {
                            funnel: {
                                x: "25%",
                                y: "20%",
                                width: "50%",
                                height: "70%",
                                funnelAlign: "left",
                                max: 1548
                            }
                        }
                    },
                    restore: {
                        show: !0,
                        title: "Restore"
                    },
                    saveAsImage: {
                        show: !0,
                        title: "Same as image",
                        lang: ["Save"]
                    }
                },
                color: "#FFF"
            },
            calculable: !0,
            series: [{
                name: "Top Categories",
                type: "pie",
                radius: ["50%", "70%"],
                center: ["50%", "57.5%"],
                itemStyle: {
                    normal: {
                        label: {
                            show: !0,
                            textStyle: {
                                color: "#FFF"
                            }
                        },
                        labelLine: {
                            show: !0,
                            lineStyle: {
                                color: "#FFF"
                            }
                        }
                    },
                    emphasis: {
                        label: {
                            show: !0,
                            formatter: "{b}\n\n{c} ({d}%)",
                            position: "center",
                            textStyle: {
                                fontSize: "17",
                                fontWeight: "500"
                            }
                        }
                    }
                },
                data: [{
                    value: 335,
                    name: "Moto Z"
                }, {
                    value: 618,
                    name: "Galaxy S7 Edge"
                }, {
                    value: 234,
                    name: "One Plus 3"
                }, {
                    value: 135,
                    name: "Mi 5"
                }, {
                    value: 956,
                    name: "iPhone 6s"
                }]
            }]
        }), t(function() {
            function a() {
                setTimeout(function() {
                    r.resize()
                }, 200)
            }
            t(e).on("resize", a), t(".menu-toggle").on("click", a)
        });
        var o = l.init(a.getElementById("timeline")),
            i = 1,
            m = {
                timeline: {
                    x: 10,
                    x2: 10,
                    data: ["2014-01-01", "2014-02-01", "2014-03-01", "2014-04-01", "2014-05-01", {
                        name: "2014-06-01",
                        symbol: "emptyStar2",
                        symbolSize: 8
                    }, "2014-07-01", "2014-08-01", "2014-09-01", "2014-10-01", "2014-11-01", {
                        name: "2014-12-01",
                        symbol: "star2",
                        symbolSize: 8
                    }],
                    label: {
                        formatter: function(e) {
                            return e.slice(0, 7)
                        },
                        textStyle: {
                            color: "#FFF"
                        }
                    },
                    checkpointStyle: {
                        color: "#6C5B7B",
                        borderColor: "#FFF",
                        label: {
                            textStyle: {
                                color: "#FFF"
                            }
                        }
                    },
                    controlStyle: {
                        normal: {
                            color: "#FFF"
                        }
                    },
                    lineStyle: {
                        color: "#FFF",
                        width: 1,
                        type: "dashed"
                    },
                    autoPlay: !0,
                    playInterval: 3e3
                },
                options: [{
                    title: {
                        text: "Browser statistics",
                        subtext: "Based on shared research",
                        x: "center",
                        textStyle: {
                            color: "#FFF"
                        },
                        subtextStyle: {
                            color: "#FFF"
                        }
                    },
                    tooltip: {
                        trigger: "item",
                        formatter: "{a} <br/>{b}: {c} ({d}%)"
                    },
                    legend: {
                        x: "left",
                        orient: "vertical",
                        data: ["Chrome", "Firefox", "Safari", "IE9+", "IE8-"],
                        textStyle: {
                            color: "#FFF"
                        }
                    },
                    color: ["#FECEA8", "#FF847C", "#F8AC6F", "#6C5B7B", "#99B898"],
                    toolbox: {
                        show: !0,
                        orient: "vertical",
                        feature: {
                            mark: {
                                show: !0,
                                title: {
                                    mark: "Markline switch",
                                    markUndo: "Undo markline",
                                    markClear: "Clear markline"
                                }
                            },
                            dataView: {
                                show: !0,
                                readOnly: !1,
                                title: "View data",
                                lang: ["View chart data", "Close", "Update"]
                            },
                            magicType: {
                                show: !0,
                                title: {
                                    pie: "Switch to pies",
                                    funnel: "Switch to funnel"
                                },
                                type: ["pie", "funnel"],
                                option: {
                                    funnel: {
                                        x: "25%",
                                        width: "50%",
                                        funnelAlign: "left",
                                        max: 1700
                                    }
                                }
                            },
                            restore: {
                                show: !0,
                                title: "Restore"
                            },
                            saveAsImage: {
                                show: !0,
                                title: "Same as image",
                                lang: ["Save"]
                            }
                        },
                        color: "#FFF"
                    },
                    series: [{
                        name: "Browser",
                        type: "pie",
                        center: ["50%", "50%"],
                        radius: "60%",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }],
                        itemStyle: {
                            normal: {
                                label: {
                                    textStyle: {
                                        color: "#FFF"
                                    }
                                },
                                labelLine: {
                                    lineStyle: {
                                        color: "#FFF"
                                    }
                                }
                            }
                        }
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }, {
                    series: [{
                        name: "Browser",
                        type: "pie",
                        data: [{
                            value: 128 * i + 80,
                            name: "Chrome"
                        }, {
                            value: 64 * i + 160,
                            name: "Firefox"
                        }, {
                            value: 32 * i + 320,
                            name: "Safari"
                        }, {
                            value: 16 * i + 640,
                            name: "IE9+"
                        }, {
                            value: 8 * i++ + 1280,
                            name: "IE8-"
                        }]
                    }]
                }]
            };
        o.setOption(m), t(function() {
            function a() {
                setTimeout(function() {
                    o.resize()
                }, 200)
            }
            t(e).on("resize", a), t(".menu-toggle").on("click", a)
        })
    })
}(window, document, jQuery);